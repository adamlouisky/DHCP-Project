...
import com.websocket.WSEndpoint;
...
public class DHCPServer extends Thread{
	...
	public static DHCPServer dhcpServer;
	...
	public DHCPServer() {
		...
		subnetMask=DHCPUtility.IPv4Str2Bytes(DHCPPUB.subnetMask);
		router=DHCPUtility.IPv4Str2Bytes(DHCPPUB.router);
		dns=DHCPUtility.IPv4Str2Bytes(DHCPPUB.dns);
	...
	public static void sendPacket(String clientIP, DHCPMessage msg) {
		byte[] payload=msg.externalize();
		assert(payload.length <= MAX_BUFFER_SIZE);
	...
	public static void broadcastPacket(DHCPMessage msg) {
		byte[] payload=msg.externalize();
		assert(payload.length <= MAX_BUFFER_SIZE);
		try {
			String broadcastIP = "255.255.255.255";
			...
			//System.out.println("Broadcasting data: " + Arrays.toString(p.getData()));
			socket.send(p);
			logger.debug("Send broadcast to {}:{}",p.getAddress(),p.getPort());

			//send to web this event
			String strEvent=createReportMsg(msg);
			WSEndpoint.sendMsg(strEvent);
   ...
   */
	...
	private static void process(byte[] msg) {
		...
		//log receive msg, send this event to web

		String strEvent=createReportMsg(request);
		WSEndpoint.sendMsg(strEvent);

		if (request.getOp() == DHCPMessage.DHCPREQUEST) {
			if (msgType == DHCPOptions.DHCPDISCOVER) {
//				byte[] offer = createOfferReply(request);
				logger.debug("DHCPDiscover: {} {}",NL ,request.toString());
				DHCPMessage offer=createOfferReply(request);
...
//				log("log.txt", "DHCPServer: Broadcasting Offer Reply" + NL);
	...
	private static DHCPMessage createOfferReply(DHCPMessage discover) {
		byte[] ip; //offer ip


		String avalIP="";
		String mac=discover.printCHAddr().toUpperCase();
		avalIP=AssignUtil.getAvailableIP(mac);
		logger.debug("AvailIP: {} --> {}",avalIP,mac);

		ip=DHCPUtility.strToIP(avalIP);
		// compare request ip, to transaction offer ip, ensure it is still
		// unique
	...
	private  static String createReportMsg(DHCPMessage msg){
		JSONObject json=new JSONObject();
		int msgType =0xFF & msg.getOptions().getOptionData(DHCPOptions.DHCPMESSAGETYPE)[0];

		json.put("msgType",msgType);
		json.put("time",System.currentTimeMillis());
		switch(msgType){
			case DHCPOptions.DHCPDISCOVER:
				String mac=msg.printCHAddr();
				json.put("mac",msg);
				AssignUtil.updateDiscover(mac);

				break;
			case DHCPOptions.DHCPACK:
				break;
			case DHCPOptions.DHCPLEASETIME:
				break;
